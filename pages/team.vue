<template>
    <div class="dark">
        ...
        <div class="flex flex-col gap-4 dark:bg-[#0b1026] text-gray-100">
            ...
            <div class="p-8 lg:p-16 rounded-3xl">
                <CardHeader>
                ...
                <CardContent class="px-8 lg:px-16">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-16">
                        <Card v-for="contributor in contributors" class="bg-[#1a1c2c] py-2 md:p-8 rounded-xl">
                            ...
                            <CardContent class="text-center">
                                <CardDescription class="text-lg commits">{{ contributor.contributions }} commits</CardDescription>
                            </CardContent>
                        </Card>
                    </div>
                </CardContent>
...
onMounted(async () => {
    ...
    contributors.value = await response.json()
        ...
        const format = ref({
            moddersteam: [
            {
                modder_username: 'Pranav Purwar',
                modder_description: 'Main modder',
                modder_img: 'https://avatars.githubusercontent.com/u/75154889?v=4',
                isTitled: true,
                isMainModder: true,
                isActive: true,
                title: 'Main modders'
        ...
        const fallback = await fetch('https://raw.githubusercontent.com/Sketchware-Pro/Sketchware-Pro/host/aboutus.json',
            {
                headers: {
                    'Accept': 'application/json'
                }
        ...
        format.value = await fallback.json()

        mainModders.value = format.value.moddersteam.filter(modder => modder.isMainModder).map(modder => {
            return {
                login: modder.modder_username,
                contributions: 0,
                avatar_url: modder.modder_img
            }
        })

        contributors.value = format.value.moddersteam.filter(modder => !modder.isMainModder).map(modder => {
            return {
                login: modder.modder_username,
                contributions: 0,
                avatar_url: modder.modder_img
